...
from base_models import CustomDataset, MultiTaskNet

import logging 
logger = logging.getLogger(__name__)


class XMultiTaskLearner(BaseEstimator, RegressorMixin):
    def __init__(self, input_size, num_classes, params, propensity_model, device, epochs=10):
        ...
        self.mu_net = MultiTaskNet(input_size, hidden_size, num_classes, dropout, shared_layers, task_layers, self.num_tasks).to(device)
        self.tau_net = MultiTaskNet(input_size, hidden_size, REGRESSION_OUTPUT_SIZE, dropout, shared_layers, task_layers, self.num_tasks).to(device)

        self.optimizer_mu = optim.Adam(self.mu_net.parameters(), lr=self.learning_rate, weight_decay=self.lambda_reg)
        self.optimizer_tau = optim.Adam(self.tau_net.parameters(), lr=self.learning_rate, weight_decay=self.lambda_reg)
        ...
        self.tau_losses = np.zeros((self.epochs, self.num_tasks))

        logging.info("Initialized XMultiTaskLearner on device: %s", device)
    ...
    def train_multi_task(self, X_control, y_control, w_control, X_treated, y_treated, w_treated, task_type="classification"):
        ...
        ipw_c = torch.tensor(self._compute_ipw_weights(X_control, treated=False), dtype=torch.float32).to(self.device)
        ipw_t = torch.tensor(self._compute_ipw_weights(X_treated, treated=True), dtype=torch.float32).to(self.device)
        ...
        for i, ((Xc, yc, _), (Xt, yt, _)) in enumerate(zip(loader_control, loader_treated)):
            ...
            m0_out, m1_out = model(Xc)
            _, m1_out_ = model(Xt)

            wc, wt = ipw_c[i * self.batch_size:(i + 1) * self.batch_size], ipw_t[i * self.batch_size:(i + 1) * self.batch_size]

            wc = torch.ones_like(yc).float().to(self.device)
            wt = torch.ones_like(yt).float().to(self.device)

            loss_c = (criterion(m0_out, yc) * wc).mean()
            loss_t = (criterion(m1_out_, yt) * wt).mean()
            reg_term = self.lambda_reg * sum(p.pow(2.0).sum() for p in model.parameters())

            loss = loss_c + loss_t + reg_term
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    def fit(self, X, y, t_ind):
        ...
        with torch.no_grad():
            logging.info("Imputing treatment effects...")
            X_c, X_t = torch.tensor(X[t_ind == 0], dtype=torch.float32).to(self.device), torch.tensor(X[t_ind == 1], dtype=torch.float32).to(self.device)
            y_c, y_t = y[t_ind == 0], y[t_ind == 1]

            probs_c = torch.softmax(self.mu_net(X_c)[1], dim=1).cpu().numpy()
            probs_t = torch.softmax(self.mu_net(X_t)[0], dim=1).cpu().numpy()

            true_probs_c = probs_c[np.arange(len(y_c)), y_c.astype(int)]
            true_probs_t = probs_t[np.arange(len(y_t)), y_t.astype(int)]

            imputed_te0 = true_probs_c - y_c
            imputed_te1 = y_t - true_probs_t

        logging.info("Training treatment effect models (tau0, tau1)...")
        for epoch in range(self.epochs):
            loss_c, loss_t, _, _ = self.train_multi_task(
                X[t_ind == 0], imputed_te0, t_ind[t_ind == 0],
                X[t_ind == 1], imputed_te1, t_ind[t_ind == 1],
                task_type="regression"
            )
            self.tau_losses[epoch] = [loss_c, loss_t]
            logging.info(f"[Epoch {epoch+1}] Tau Loss (C): {loss_c:.4f}, Tau Loss (T): {loss_t:.4f}")
    ...
    def predict_outcome(self, X, y, t_ind, treated_flag=True):
        ...
        # Prepare DataLoader
        loader = self._get_loader(X, y, t_ind)
    ...
    def predict_effect(self, X):
        ...
        with torch.no_grad():
            ...
            logits_control, logits_treated = self.mu_net(X_tensor)  # mu0(X), mu1(X); both shape (N, 4)
            predictions_treated = torch.softmax(logits_treated.cpu(), dim=1).numpy()  # Probabilities for treated
            predictions_control = torch.softmax(logits_control.cpu(), dim=1).numpy()  # Probabilities for control

            # Step 2: Compute imputed counterfactual outcomes
            imputed_logits_control, imputed_logits_treated = self.mu_net(X_tensor)
            imputed_control = torch.softmax(imputed_logits_control.cpu(), dim=1).numpy()  # mu0(X) as counterfactual for treated
            imputed_treated = torch.softmax(imputed_logits_treated.cpu(), dim=1).numpy()  # mu1(X) as counterfactual for control
            ...
            treatment_effect_per_class = propensity_scores[:, None] * D1 + (1-propensity_scores[:, None])* D0 # Shape: (N, 4)

            # Step 5: Use meta-learner τ(X) to refine treatment effect estimates per class
            logits_tau_control, logits_tau_treated = self.tau_net(X_tensor)
            tau0_predictions = torch.softmax(logits_tau_control.cpu(), dim=1).numpy()  # τ₀(X), shape (N, 4)
            tau1_predictions = torch.softmax(logits_tau_treated.cpu(), dim=1).numpy()  # τ₁(X), shape (N, 4)
    ...
    def save(self, path):
        ...
        torch.save(self.tau_net.state_dict(), os.path.join(path, "tau_net.pth"))
        ...
        }, os.path.join(path, "xmultitasklearner_config.pkl"))
        
        logging.info(f"✅ XMultiTaskLearner saved to {path}")
    
    def load(self, path, device):
        # Load config
        state = joblib.load(os.path.join(path, "xmultitasklearner_config.pkl"))
        ...
        self.mu_net = MultiTaskNet(
            self.input_size, self.params['hidden_size'], self.num_classes,
            self.params['dropout_rate'], self.params['num_shared_layers'],
            self.params['num_task_specific_layers'], self.params['num_tasks']
        ).to(self.device)

        self.tau_net = MultiTaskNet(
            self.input_size, self.params['hidden_size'], 1,
            self.params['dropout_rate'], self.params['num_shared_layers'],
            self.params['num_task_specific_layers'], self.params['num_tasks']
        ).to(self.device)
        ...
        self.tau_net.load_state_dict(torch.load(os.path.join(path, "tau_net.pth"), map_location=self.device))
